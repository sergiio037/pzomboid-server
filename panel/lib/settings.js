 ...
 * Devuelve { text, applied, skipped } para poder avisar de lo que no existia.
 */
function applyIni(text, changes) {
  let out = String(text || '');
  const applied = [];
  const skipped = [];
  ...
  for (const [key, rawVal] of Object.entries(changes || {})) {
    ...
    if (!re.test(out)) { skipped.push(key); continue; }
...
module.exports = { parseIni, applyIni };
